...
function CollaborativeEditor(ctl, url, session_id) {
	...
	console.debug("Opening Signalr", url);

	ce._hub = $.connection.documentShare;
	
	$.connection.hub.start().done(function (event) {
		ce._onSocketOpen(event);
		ce._localUser = 1;//$.connection.hub.id;
		console.debug("local id:" + ce._localUser);
	});
	...
	//$.connection.error(function (event) { ce._onSocketConnectionLost(); });
	//ce._socket.onmessage = function(event) { ce._onSocketMessage(event); };
	ce._hub.client.addTransform = function (event) { ce._onSocketMessage(event); };
...
CollaborativeEditor.prototype._handleUpdates = function() {
	...
	//console.debug("diffs okay");

	var offset = 0;
	for (var diffIndex in diffs) {
		//console.debug("looping through diffs");
		...
		if (diffType == 1) {
			...
			//console.debug("unrecognized diff");
			offset += diffText.length;
		}
	}
	//console.debug("done with diffs");
...
CollaborativeEditor.prototype._onSocketMessage = function(event) {
	...
	//console.debug('hey i recieved something' + event);
	...
	for (var commandIndex in jsonData) {
		...
		if (command == "assign_uid") {
			...
			if (args[0] != this._localUser) {
				...
				var executedRequest = this._state.execute(request);
				console.debug("executed: " + executedRequest);
				this._updateControl(executedRequest);
			}
